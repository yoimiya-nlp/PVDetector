            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1']>best_f1:
                        ...
                        checkpoint_prefix = 'checkpoint-best-f1-reveal-03'
                        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))                        
                        if not os.path.exists(output_dir):
                            os.makedirs(output_dir)                        
                        model_to_save = model.module if hasattr(model,'module') else model
                        pretrain = model.encoder
                        pretrain.save_pretrained("pretrain_model")
...
def main():
    ...
    # split dataset
    #Preprocess(tokenizer, args, file_path=args.pvdata_data_file)
    #exit(0)
    ...
    if args.do_train:
        ...
        #持续训练
        checkpoint_prefix = 'checkpoint-best-f1-reveal_03/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
    ...
    results = {}
    if args.do_test:
        checkpoint_prefix = 'checkpoint-best-f1-qemu-02/model.bin'
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
        pvdetector_model.load_state_dict(torch.load(output_dir))
        pvdetector_model.to(args.device)
