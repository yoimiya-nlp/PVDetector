...
def generate_CSG_df_subgraph_php(root_node,index_to_code,states,code,new_tokens):
    ...
    special_situaion = ['function_call_expression']
    states=states.copy()
    if (len(root_node.children)==0 or root_node.type.find('string') != -1) and root_node.type!='comment':
        ...
        if value is None:
            ...
            temp, states, new_tokens = generate_CSG_df_subgraph_php(value, index_to_code, states, code, new_tokens)
            csg_df += temp
        ...
        csg_df=[]
        temp,states,new_tokens=generate_CSG_df_subgraph_php(right_nodes,index_to_code,states,code, new_tokens)
        csg_df+=temp
        ...
        for child in root_node.children:
            ...
            if child.type == 'parenthesized_expression':
                temp, states,new_tokens = generate_CSG_df_subgraph_php(child,index_to_code,states,code,new_tokens)
                current_states = states.copy()         
                csg_df += temp
                continue
            if child.type == 'else_clause':     
                haveelse_tag=True
                temp,new_states,new_tokens=generate_CSG_df_subgraph_php(child,index_to_code,current_states,code,new_tokens)
                csg_df+=temp
        ...
        if root_node.type == 'function_call_expression':
            ...
            input_func_list = ["fgets", "fscanf", "_GET", "_POST"]
            mem_func_list = ["memcpy", "memset", "memmove", "str_repeat"]
            if function_name in input_func_list:
                ...
                temp, states, new_tokens = generate_CSG_df_subgraph_php(argument_list, index_to_code, states, code, new_tokens)
                csg_df += temp
...
def generate_CSG_cf_subgraph_php(root_node):
    if_statement=['if_statement']
    for_statement=['for_statement']
    switch_statement=['switch_statement']
    while_statement=['while_statement']
    ...
    if root_node.type in if_statement:
        ...
        for child in root_node.children:
            temp = generate_CSG_cf_subgraph_php(child)
            csg_cf += temp
...
def generate_CSG_vul_subgraph_php(root_node, index_to_code, states, code, new_tokens):
    function_name = ['function_definition']
    for_statement=['for_statement']
    while_statement=['while_statement']
    do_first_statement=['do_statement']
    api_relation = ['function_call_expression']
    ...
    if root_node.type in function_name:
        ...
        csg_vul = []
        for child in root_node.children:
            temp, states, new_tokens = generate_CSG_vul_subgraph_php(child, index_to_code, states, code, new_tokens)
            csg_vul += temp
        ...
        if function_name in input_func_list:
            csg_vul = []
            argument_list = root_node.children[1]
            temp, states, new_tokens = generate_CSG_vul_subgraph_php(argument_list, index_to_code, states, code, new_tokens)
            csg_vul += temp
